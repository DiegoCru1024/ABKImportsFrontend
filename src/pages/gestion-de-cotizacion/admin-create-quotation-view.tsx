...
import { useCreateQuotationAdministrador } from "@/hooks/use-quation";
...
import { VariantRow } from "@/pages/cotizacion-page/components/ui/variant-row";
import { ProductSummary } from "@/pages/cotizacion-page/components/ui/product-summary";
import { ProductsTable } from "@/pages/cotizacion-page/components/table/products-table";
import type {
  ProductVariant,
  ProductWithVariants,
} from "@/pages/cotizacion-page/utils/types/local.types";

import { productSchema } from "@/pages/cotizacion-page/utils/types/schemas";
import { toAPI } from "@/pages/cotizacion-page/utils/types/mappers";
import { UserSelectionModal } from "./components/modals/user-selection-modal";
import type { UserProfile } from "@/api/interface/user";

export default function AdminCreateQuotationView() {
  ...
  const createQuotationMut = useCreateQuotationAdministrador();
  ...
  const handleSubmitQuotation = async (saveAsDraft: boolean = false) => {
    ...
    if (!selectedUser) {
      toast.error("Debe seleccionar un cliente para asignar la cotización");
      return;
    }
    ...
    try {
      ...
      const basePayload = toAPI.quotation(
        productosConUrls,
        service,
        saveAsDraft
      );

      // 3. Agregar el ID del cliente
      const payload = {
        ...basePayload,
      ...
      // 4. Enviar cotización al backend
  ...
  //* Handlers para el modal de selección de usuario
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="border-border/60 bg-background/80 backdrop-blur-sm sticky top-0 z-10">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-sm font-normal text-gray-500 dark:text-gray-400 mt-1">
                Crea una cotización y asígnala a un cliente específico
              </p>
            </div>
          </div>
        ...
        {/* Selector de Cliente */}
        <div className="w-full px-6 py-4 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 border-b border-border/60">
                ...
                <div className="flex items-center justify-between bg-white dark:bg-gray-800 p-3 rounded-lg border border-blue-200 dark:border-blue-700">
                  <div>
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      {selectedUser.email} - {selectedUser.company_name}
                    </p>
                  </div>
                  <Button
                    variant="outline"
      ...
      <div className="w-full p-6">
        ...
        <div className="flex justify-end gap-4 pt-8">
          ...
          <Button
            ...
            disabled={isLoading || productos.length === 0 || !selectedUser}
          ...
          <ConfirmDialog
            ...
            description={`¿Está seguro de enviar la cotización con ${
              productos.length
            } producto${productos.length !== 1 ? "s" : ""} al cliente ${
