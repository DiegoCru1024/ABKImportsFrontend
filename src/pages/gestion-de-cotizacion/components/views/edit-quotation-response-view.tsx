...
export default function EditQuotationResponseView() {
    ...
    console.log("Detalles de la cotizacion cruda quotationDetail:", quotationDetail);
    ...
    console.log("Detalles de la respuesta de quotationDetail que es responseDetails:", responseDetails);
    ...
    useEffect(() => {
        if (responseDetails && quotationDetail && !isDataInitialized) {
            ...
            if (responseDetails.serviceType === "COTIZACION DE ORIGEN") {
                console.log("la respuesta es de cotizacion de origen !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                ...
                if (resData?.resumenInfo?.totalExpress !== undefined) {
                    console.log("este es el total exprere lleagdo a edit quotaiton reponse ", resData.resumenInfo.totalExpress);
                ...
                const productsWithData = responseDetails.products.map(
                    (respProduct: any) => {
                        ...
                        return {
                            ...
                            quantityTotal: quotProduct?.quantityTotal || 0,//falta en el json devolver esto
                            ...
                            // En el useEffect de inicialización, línea donde mapeas las variantes:
                            variants:
                                respProduct.variants?.map((respVar: any) => {
                                    ...
                                    return {
                                        ...
                                        price: parseFloat(respVar.pendingPricing?.unitPrice) || 0, // ← ASEGURAR que sea número
                                        priceExpress: parseFloat(respVar.pendingPricing?.expressPrice) || 0, // ← ASEGURAR que sea número
                                        weight: 0,
                                        cbm: 0,
                                        express: 0,
                                    };
    ...
    const calculateProductAggregatedData = useCallback(
        (product: any) => {
            ...
            console.log(`🔍 Producto ${product.id} - Variantes seleccionadas:`, selectedVariants);

            const priceData = selectedVariants.reduce(
                (acc: any, variant: any) => {
                    const price = parseFloat(variant.price) || 0;
                    const priceExpress = parseFloat(variant.priceExpress) || 0;
                    ...
                    console.log(`  Variante ${variant.id}:`, {
                        ...
                        totalExpress: priceExpress * quantity,
                    });

                    return {
                        totalPrice: acc.totalPrice + (price * quantity),
                        totalQuantity: acc.totalQuantity + quantity,
                        totalExpress: acc.totalExpress + (priceExpress * quantity),
                    };
                },
                {
                    totalPrice: 0,
            ...
            console.log(`✅ Producto ${product.id} - Totales calculados:`, priceData);

            return {
                totalPrice: Number(priceData.totalPrice.toFixed(2)),
                totalWeight: product.packingList?.weightKg || parseFloat(product.weight) || 0,
                totalCBM: product.packingList?.cbm || parseFloat(product.volume) || 0,
                totalQuantity: Number(priceData.totalQuantity),
                totalExpress: Number(priceData.totalExpress.toFixed(2)),
            };
        },
    ...
    const pendingViewTotals = useMemo(() => {
        console.log("🔍 Calculando pendingViewTotals");
        console.log("productsAggregatedData:", productsAggregatedData);
        console.log("responseDetails:", responseDetails);

        // CASO 1: Usar resumenInfo del backend si está disponible y productsAggregatedData está vacío
        // Esto pasa en la carga inicial antes de que el usuario edite nada
        const hasAggregatedData = Object.keys(productsAggregatedData).length > 0;

        if (!hasAggregatedData && responseDetails?.responseData?.resumenInfo) {
            const resumenInfo = responseDetails.responseData.resumenInfo;
            console.log("✅ Usando resumenInfo del backend (carga inicial):", resumenInfo);

            return {
                totalItems: resumenInfo.totalQuantity || 0,
                totalProducts: responseDetails.products?.length || 0,
                totalCBM: resumenInfo.totalCBM || 0,
                totalWeight: resumenInfo.totalWeight || 0,
                totalPrice: resumenInfo.totalPrice || 0,
                totalExpress: resumenInfo.totalExpress || 0,
                grandTotal: (resumenInfo.totalPrice || 0) + (resumenInfo.totalExpress || 0),
            };
        }

        // CASO 2: Recalcular desde productsAggregatedData (usuario editando)
        console.log("🔄 Recalculando desde productsAggregatedData");
        ...
        const totals = selectedProducts.reduce(
            (acc, [, data]) => ({
                totalItems: acc.totalItems + (Number(data.totalQuantity) || 0),
                totalProducts: acc.totalProducts + 1,
                totalCBM: acc.totalCBM + (Number(data.totalCBM) || 0),
                totalWeight: acc.totalWeight + (Number(data.totalWeight) || 0),
                totalPrice: acc.totalPrice + (Number(data.totalPrice) || 0),
                totalExpress: acc.totalExpress + (Number(data.totalExpress) || 0),
            }),
            {
                totalItems: 0,
                totalProducts: 0,
        ...
        const result = {
            ...totals,
            grandTotal: totals.totalPrice + totals.totalExpress,
        };

        console.log("📊 Resultado final:", result);

        return result;
    }, [productsAggregatedData, quotationForm.productQuotationState, responseDetails]);
    ...
    const totalImportCosts = useMemo(() => {
        ...
        const applyExemption = (value: number, exempted: boolean) => (exempted ? 0 : value);

        if (isMaritime) {
            const servicioConsolidado = applyExemption(
                (serviceFields.servicioConsolidado || 0) * 1.18,
                quotationForm.exemptionState.servicioConsolidadoMaritimo
            );
            const gestionCertificado = applyExemption(
                (serviceFields.gestionCertificado || 0) * 1.18,
                quotationForm.exemptionState.gestionCertificado
            );
            const servicioInspeccion = applyExemption(
                ((serviceFields.inspeccionProductos || 0) + (serviceFields.inspeccionFabrica || 0)) * 1.18,
                quotationForm.exemptionState.servicioInspeccion
            );
            ...
            const totalDerechos = applyExemption(
                calculations.totalTaxes || 0,
                quotationForm.exemptionState.totalDerechos
            );
            ...
            const servicioConsolidado = applyExemption(
                (serviceFields.servicioConsolidado || 0) * 1.18,
                quotationForm.exemptionState.servicioConsolidadoAereo
            );
            const separacionCarga = applyExemption(
                (serviceFields.separacionCarga || 0) * 1.18,
                quotationForm.exemptionState.separacionCarga
            );
            const seguroProductos = applyExemption(
            ...
            if (isExpressConsolidatedPersonal) {
                additionalExpenses =
                    applyExemption(
                        ...
                        quotationForm.exemptionState.fleteInternacional || false
                    ) +
                    applyExemption(
                        quotationForm.dynamicValues.desaduanaje || 0,
                        quotationForm.exemptionState.desaduanaje || false
                    );
            } else if (isExpressConsolidatedSimplificada) {
                additionalExpenses =
                    applyExemption(
                        calculations.totalTaxes || 0,
                        quotationForm.exemptionState.adValoremIgvIpm || false
                    ) +
                    applyExemption(
                ...
                additionalExpenses =
                    applyExemption(
                        ...
                        quotationForm.exemptionState.adValoremIgvIpmDescuento || false
                    ) +
                    applyExemption(
                        quotationForm.dynamicValues.flete || 0,
                        quotationForm.exemptionState.fleteInternacional || false
            ...
            if (isPendingView) {
                ...
                const importCostsData = {
                    expenseFields: {
                        ...
                        inspeccionProductos: serviceCalculationsData.serviceFields.inspeccionProductos,
                        servicioTransporte:
                        ...
                        gestionCertificado: serviceCalculationsData.serviceFields.gestionCertificado,
                        ...
                        desaduanaje: isExpressConsolidatedSimplificada
                            ? calculationsData.dynamicValues.desaduanaje || 0
                            : 0,

                        // Solo para consolidado express personal
                        addvaloremigvipm: isExpressConsolidatedPersonal
                            ? calculations.totalTaxes || 0
                            : 0,

                        desadunajefleteseguro: isExpressConsolidatedPersonal
                            ? calculationsData.dynamicValues.desaduanaje +
                            calculationsData.dynamicValues.flete +
                            calculationsData.dynamicValues.seguro
                            : 0,
                        ...
                        fleteInternacional: isExpressConsolidatedSimplificada || isExpressConsolidatedGrupal
                            ? calculationsData.dynamicValues.flete || 0
                            : 0,
                    },
