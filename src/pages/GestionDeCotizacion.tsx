...
export default function GestionDeCotizaciones() {
  const [mainTab, setMainTab] = useState<"solicitudes" | "detalles">(
    "solicitudes"
  );
      ...
      cell: ({ row }) => (
        <div>
          ...
          <div className="text-sm text-gray-500">
            {row.original.comentarioCliente}
          </div>
        </div>
      ),
    },
  ...
  const handleSaveResponse = () => {
    if (currentProduct) {
      setProductos((prev) =>
        prev.map((p) =>
          p.id === currentProduct.id ? { ...p, ...responseForm } : p
        )
      );
    }
    setDialogOpen(false);
    ...
    <div className="min-h-screen overflow-x-hidden bg-gray-100 border-t-2 border-b-2 border-gray-200">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        ...
        <DialogContent className="max-w-lg">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setResponseForm({
                    ...responseForm,
                    estadoRespuesta: value as ProductoItem["estadoRespuesta"],
                  })
                }
              >
          ...
          <DialogFooter>
            <DialogClose asChild>
              <Button type="submit" onClick={handleSaveResponse}>
                Guardar
              </Button>
            </DialogClose>
          </DialogFooter>
